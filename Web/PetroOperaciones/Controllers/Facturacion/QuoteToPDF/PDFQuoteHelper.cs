...
{
    public class PDFQuoteHelper
    {

        public static String TEMPLATE_INVOICE_FORMAT = "Formato-Cotizacion-Petrocargo.pdf";
        public static String TEMPLATE_BIG_INVOICE_FORMAT = "Formato-Cotizacion-Continuo.pdf";
        ...
        private CotizacionEncabezado invoiceHeader;
        private String currencyType;

        public PDFQuoteHelper(String pathOfTemplates, String pathOfInvoiceTmp, ArrayList vInvoiceFiles)
        {
        ...
        public void createInvoicesPages(IMasterEntity invoiceHeader, PdfPTable mainTable, String currencyType)
        {
            this.mainTable = mainTable;
            this.invoiceHeader = (CotizacionEncabezado)invoiceHeader;
        ...
        private void makePage(PdfPTable subtable, bool isTotaSubTable)
        {
            ...
            using (FileStream fs = new FileStream(invoiceAuxFile, FileMode.Create, FileAccess.Write, FileShare.None))
            {
                using (PdfStamper stamper = new PdfStamper(reader, fs))
                {
                    ...
                    QuoteMasterToPDF invoiceMasterToPDF = new QuoteMasterToPDF();
                    invoiceMasterToPDF.fillInvoiceMaster(invoiceHeader, stamper);

                    if (isTotaSubTable)
                    {
                        invoiceMasterToPDF.fillTotalsMaster(invoiceHeader, stamper, currencyType);
                    }
